...
from ztfsmp.pipeline_utils import (
    run_and_log,
    dump_timings,
    dump_timings_reduce,
    load_timings,
    quadrants_from_band_path,
    noprocess_quadrants)
...
def map_op(exposure_name, wd, name, filtercode, func, args, op_args):
    try:
        ...
        lightcurve_path = wd / name / filtercode
        exposure_path = wd / name / filtercode / exposure_name
...
def reduce_op(wd, name, filtercode, func, save_stats, args, op_args):
    ...
    #         columns=['result', 'time_end', 'time_start', 'worker_id'])
    #     results_df.to_csv(folder / f"results_{func}.csv", index=False)
...
def main():
    ...
    for ztfname in ztfnames:
        for filtercode in filtercodes:
            ...
            # Save pipeline arguments and description. Since runs can be
            # launched sequencialy without intermediate cleaning, an unique
            # filename is created per run. If a clean operation is scheduled,
            # overwrite the first ones (they will be ignored)
